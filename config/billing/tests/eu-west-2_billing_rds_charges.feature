Feature: Test AWS RDS bills calculated correctly
  ...
  Then the bill, including VAT, should be £6.77
...
####
  ...
  Examples:
    ...
    | postgres small-11 | 2021-05-01 | 2021-06-01 | May 2021 | 42.05 |

# Calculation from https://calculator.aws/#/createCalculator/RDSPostgreSQL:
# 1 instance(s) x 0.041 USD hourly x 744 hours in a month = 30.504 USD
# 100 GB per month x  0.133 USD x 1 instances = 13.30 USD (Storage Cost)
# Total cost in GBP, including VAT = (0.8*(13.30 + 30.504))*1.2 = £42.05
...
####
  ...
  Then the bill, including VAT, should be £5954.67

  # Calculation for the above:
  # Note that $3.385 is the price per hour but bills are calculated to the second
  #
  #Calculation
  # (((521940)*(3.385/3600)) +  ((3600)*(3.385/3600)) + ((2066460)*(3.296/3600))) + ((2 * ((0.266*(2097152/1024))) + (0.266*(10485760/1024))) * (2678400/2678401)) * 0.8 * 1.2 gives 5954.67 (when correctly rounded)
  #
  # # Calculated cost in USD between '2018-11-01 00:00:00' and '2020-06-07 00:59' (instance followed by storage) is ((521940)*(3.224/3600)) + ((521940/2678401)*((0.253*(2097152/1024)))) = 568.39702509
...
#  Then a tenant should have a small-10.5 with a valid_from of '2017-09-07 18:00:47+00'
  ...
  Then the bill, including VAT, should be £1507.92
...
#01/06/2020 00:00:00	07/06/2020 00:59:00	521940	2097152	3.296	0.266	0.0009155555556	477.92	382.336	458.8032	544.768	435.8144	522.97728	981.78048
#2020-06-07 00:59	2020-06-07 01:59	3600	2097152	3.296	0.266	0.0009155555556	3.296	2.6368	3.16416	544.768	435.8144	522.97728	526.14144
