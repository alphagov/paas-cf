...
get_compose_secrets() {
  ...
  if aws s3 ls "${secrets_uri}" > /dev/null ; then
    ...
    compose_email=$("${SCRIPT_DIR}"/val_from_yaml.rb compose_email "${secrets_file}")
    compose_password=$("${SCRIPT_DIR}"/val_from_yaml.rb compose_password "${secrets_file}")
