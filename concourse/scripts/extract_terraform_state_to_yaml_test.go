			...
			cmdInput = `
{
    ...
    "modules": [
		...
		It("Exits non-zero with an error on STDERR", func() {
			Eventually(session).Should(gexec.Exit(0))
			Expect(session.Out).To(gbytes.Say("terraform_outputs_bar_dns_name: bar.example.com"))
			Expect(session.Out).To(gbytes.Say("terraform_outputs_bar_elastic_ip: 10.210.220.248"))
			Expect(session.Out).To(gbytes.Say("terraform_outputs_bar_security_group_id: sg-123456789"))
		})
	})
})
